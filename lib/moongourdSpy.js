...
module.exports = function MoongourdParser(mod) {
	...
	const getLink = (publisher, playerName, continent, boss) => {
		...
		return `https://kabedon.moongourd.com/api/mg/search.php?region=${region}&zone=${continent}&boss=${boss}&ver=1&name=${playerName}&page=1`
	};
	...
	const analyzeChar = async (charName, continentId, bossId, dungeonName) => {
		...
		let payload = await requestData(link);
		...
		if (!Array.isArray(payload) || !Array.isArray(payload[1])) {
			mod.command.message("Invalid API response! Server unavailable or mod need update.")
			return;
		}
	...
	mod.hook('S_USER_PAPERDOLL_INFO', 11, event => {
		if (mod.settings.autoCheckAtInspect && event.name != mod.game.me.name) {
			...
			if (throttleList.has(event.nane) && Date.now() - throttleList.get(event.name) < THROTTLE_REPEAT_DURATION) return;

			throttleList.set(event.name, Date.now());

			mod.settings.continentsToCheck.forEach(x => { analyzeChar(event.name, x.continent, x.boss, x.name); });
		}
	});
	...
	mod.command.add("ms", {
		$none() {
			...
			mod.command.message(`Auto inspect was ${mod.settings.enabled ? "en" : "dis"}abled`);
		},
		i(name) {
			mod.settings.continentsToCheck.forEach(x => { analyzeChar(name, x.continent, x.boss, x.name); });
