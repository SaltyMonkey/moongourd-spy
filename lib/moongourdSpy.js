...
module.exports = function MoongourdParser(mod) {
	...
	const sendOutput = (charName, dungeonName, res) => {
		if (mod.settings.simplifiedOutput) {
			mod.command.message(`${charName}/${dungeonName}| Min=${res.playerDpsMin} Avg=${res.playerDpsAvg} Max=${res.playerDpsMax}| ${res.status}`);
		}
		else {
			mod.command.message(`Highlight ${charName} in ${dungeonName} (latest 25 runs):`)
			mod.command.message(`- Player DPS: Min=${res.playerDpsMin} Avg=${res.playerDpsAvg} Max=${res.playerDpsMax} ${res.status}`);
