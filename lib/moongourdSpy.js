...
module.exports = function MoongourdParser(mod) {
	...
	const getLink = (serverId, playerName, continent, boss, linkVersionModifier) => {
		let region = "EU";
		switch (serverId) {
			case (43):
			case (42): region = "NA"; break;
		}
		return `https://moongourd.com/api/mg/search.php?region=${region}&zone=${continent}&boss=${boss}&ver=${linkVersionModifier || 1}&name=${playerName}&page=1`
	};
	...
	const analyzeChar = async (charName, continentId, bossId, dungeonName, expectedDps, linkVersionModifier) => {
		const link = getLink(mod.serverId, charName, continentId, bossId, linkVersionModifier);
	...
	const startWrappedCheck = async (name) => {
		if (mod.settings.keepContinentsSequence)
			for(const x of mod.settings.continentsToCheck) await analyzeChar(name, x.continent, x.boss, x.name, x.expectedDpsAvg, x.phase);
		else 
			for(const x of mod.settings.continentsToCheck) analyzeChar(name, x.continent, x.boss, x.name, x.expectedDpsAvg, x.phase);
	};
